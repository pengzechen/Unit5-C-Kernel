## Lesson 09 Physical Memory Manager 物理内存管理器
...
- `PHYS_TO_VIRT` / `VIRT_TO_PHYS`
  ...
  - 所以转换只是加减一个常数 —— 但这个宏必须用对，否则就是 Lesson 02 提到的 PA/VA 混淆 bug
- 多核安全
  - `pmm_alloc_page` 和 `pmm_free_page` 用 `spin_lock_irqsave` 保护 —— Lesson 08 的直接应用
