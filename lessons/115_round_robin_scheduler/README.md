...
### 代码
    ...
    void schedule(void) {
        ...
        if (prev != next)
            arch_task_switch(&prev->sp, next->sp);     // Lesson 114
    }
...
- 调度的触发时机
  ...
  - **sched_tick()**：定时器中断设 `need_resched`，异常返回时调度 —— 抢占式（Lesson 116）
- 就绪队列的数据结构
  - Avatar OS 用 `list_t`（双向链表，Lesson 98 的 `container_of` 实际应用）
  - 入队 O(1)（`list_add_tail`），出队 O(1)（`list_first_entry` + `list_del`）
  - 多核时每核一个就绪队列（通过 `cpu_rq(cpu_id)` 访问）—— Lesson 120 扩展
- 调度器的锁
  - 就绪队列在中断上下文（timer tick）和任务上下文都可能被修改
  - 必须用 `spin_lock_irqsave` 保护 —— Lesson 104 的直接应用
