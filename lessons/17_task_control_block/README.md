## Lesson 17 Task Control Block 任务控制块
...
- `list_node_t run_node` —— 侵入式链表
  ...
  - 这就是 Lesson 02 介绍的 `container_of` 宏的实际应用

### 课堂讨论

- 为什么 `sp` 字段必须是 `task_t` 的第一个成员？（提示：汇编代码直接用偏移 0 访问）
- 如果 `stack_base` 指向的是 PMM 分配的物理页，任务栈会在哪个虚拟地址？（提示：Lesson 12 的线性映射）
- `TASK_MAX = 64` 够用吗？busybox shell 会创建多少任务？

### 课后练习

- 测试：创建 3 个内核任务，每个打印自己的名字和 ID，观察输出顺序
- 扩展：阅读 `task_t` 的完整定义，标注哪些字段是 Lesson 21（用户态）才用到的
